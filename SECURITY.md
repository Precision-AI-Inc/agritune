...
Security fixes are provided for the latest released version of `pai-agritune`.
...
- The affected package version (`pip show pai-agritune`)
...
- Unauthorized access to data, features, or results served through the API
- Arbitrary code execution triggered by a crafted input payload, config file, or checkpoint
- Leakage of encoder API credentials (keys, tokens) through logs, tracking backends, or run artifacts
...
```bash
pip install --upgrade pai-agritune
```
