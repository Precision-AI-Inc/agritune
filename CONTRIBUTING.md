...
Thank you for your interest in contributing to AgriTune.
...
2. Make your changes.
3. Add or update tests — coverage must remain at or above 90%. New code that touches the encoder must be testable through `FakeEncoderBackend`; do not require a live API key for unit tests.
...
`Trainer` and `Decoder` implementations must never call the encoder API directly — everything goes through `FeatureProvider`. Reviews should reject any PR that imports `precisionai.agritune.encoder` from `precisionai.agritune.training` or `precisionai.agritune.tasks`.

## Tests

```bash
pytest                                    # run all tests with coverage report
pytest tests/unit/test_features.py        # run a specific file
pytest -k "cache"                         # run tests matching a pattern
pytest -m "not integration"               # skip integration tests
```

Tests live in `tests/` and mirror the package structure (`tests/unit`, `tests/integration`, `tests/distributed`, `tests/fixtures`). The coverage threshold (90%) is enforced both by `pytest` directly and by the pre-commit hook.
